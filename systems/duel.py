...
class DuelSystem:
    ...
    def info(self, message):
        self.bot.reply_to(
            ...
            f"\n\n"
            f"В этой статье мы расскажем о системе дуэлей."
            f"\n\n"
            f"• *Дуэли* — это PvP-сражения между двумя игроками, которые могут быть инициированы с помощью команды /duel @username в Обороне шиюй."
            f"\n\n"
            f"• *Как работают дуэли?*"
            f"\n"
            f"После вызова игрока на дуэль, тот может отказаться или принять вызов. Если игрок принял вызов - начинается бой."
            f"\n"
            f"Игроки по очереди выбирают действия: атаковать или попытаться сбежать."
            f"\n\n"
            f"• *Атака* — при выборе этой опции игрок наносит урон противнику, который рассчитывается на основе его характеристик и случайного множителя."
            f"\n"
            f"Есть шанс нанести критический урон или промахнуться."
            f"\n\n"
            f"• *Побег* — при попытке сбежать есть высокий шанс провала. Если попытка успешна, игрок покидает бой, и никто из игроков не получает победу."
            f"\n\n"
            f"• *Характеристики:*"
            f"\n"
